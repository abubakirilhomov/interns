...
const Dashboard = () => {
  ...
  // Calculate total lessons visited from lessonsVisited array
  const totalLessonsVisited = user?.lessonsVisited
    ? user.lessonsVisited.reduce((total, entry) => total + (entry.count || 0), 0)
    : 0;

  // Calculate lessons for the current month
  ...
  const monthlyLessons = user?.lessonsVisited
    ? user.lessonsVisited.reduce((total, entry) => {
        // Assuming lessonsVisited entries are linked to lessons with timestamps
        // Filter by lessons from the current month (requires lesson data)
          ...
          <div className="card-body">
            ...
            {user?.feedbacks && user.feedbacks.length > 0 ? (
              <div className="space-y-3">
                {user.feedbacks.slice(-3).reverse().map((feedback, index) => (
                  <div key={index} className="flex items-start space-x-3 p-3 bg-base-200 rounded-lg">
                    <div className="flex-shrink-0">
                      <div className="badge badge-warning">{feedback.stars}⭐</div>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-base-content/80">
                        {feedback.feedback || 'Без комментария'}
                      </p>
            ...
            {user?.lessonsVisited && user.lessonsVisited.length > 0 ? (
              <div className="space-y-3">
                {Object.entries(
                  ...
                  .slice(0, 6)
                  .map(([month, count]) => (
                    <div key={month} className="flex items-center justify-between p-3 bg-base-200 rounded-lg">
