...
const API_URL = import.meta.env.VITE_API_URL1 || 'http://localhost:8000/api';
...
export const loginIntern = createAsyncThunk(
  ...
  async ({ username, password }, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.post('/interns/login', { username, password });
...
export const updateProfile = createAsyncThunk(
  ...
  async (profileData, { rejectWithValue, getState }) => {
    try {
      ...
      const response = await axios.put(`/interns/${auth.user._id}`, profileData);
      return response.data;
    } catch (error) {
...
export const fetchProfile = createAsyncThunk(
  ...
  async (_, { rejectWithValue, getState }) => {
    try {
      ...
      return rejectWithValue(error.response?.data?.error || 'Ошибка при загрузке профиля');
    }
  }
);
